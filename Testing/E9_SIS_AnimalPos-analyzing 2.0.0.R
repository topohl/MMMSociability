...
for (batch in batches) {
  for (cageChange in cageChanges) {
    ...
    data_preprocessed$Bin30min <- floor_date(data_preprocessed$DateTime, unit = "30 minutes")

    unique_animals <- unique(data_preprocessed$AnimalID)
    unique_bins <- sort(unique(data_preprocessed$Bin30min))
    expand_bin_animal <- expand.grid(Bin30min = unique_bins, AnimalID = unique_animals, stringsAsFactors = FALSE)
    ...
    cat("Filled bins rows:", nrow(filled_bins), "\n")
    ...
    phases_column <- c()
    for(i in 1:max(length(active_phases_number), length(inactive_phases_number))){
    ...
    for (system_id in unique_systems) {
      cat("\n--- SYSTEM:", system_id, "---\n")
      data_system <- filled_bins %>% filter(System == system_id)
      cat("data_system rows:", nrow(data_system), "\n")
      ...
      for (phase in phases) {
        ...
        for (phase_number in phase_numbers) {
          ...
          data_system_phase <- data_system %>%
            ...
            filter(!is.na(DateTime)) # <-- Fix: remove rows with NA DateTime to prevent error in funcs
          ...
          while(current_row < total_rows){
            ...
            count_proximity_list <- update_proximity(prev_animal_list, animal_list, count_proximity_list, elapsed_seconds)
            count_position_list <- update_position(prev_animal_list, animal_list, count_position_list, elapsed_seconds)
            total_proximity_list <- update_total_proximity(prev_animal_list, animal_list, total_proximity_list, elapsed_seconds)
            count_movement_list <- update_movement(prev_animal_list, animal_list, count_movement_list, elapsed_seconds)
