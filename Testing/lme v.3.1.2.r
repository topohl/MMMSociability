  ...
  if (nrow(phase_avg_means_tbl) == 0 && nrow(phase_avg_contr_tbl) == 0) {
    ...
    if (nrow(phase_avg_contr_tbl) > 0) {
      forest_phaseavg <- ggplot(
        ...
        scale_color_manual(values = c("RES-CON" = "#3e3c6f", "SUS-CON" = "#e63a47", "SUS-RES" = "grey60")) +
        scale_shape_manual(values = c("RES-CON" = 16, "SUS-CON" = 16, "SUS-RES" = 16)) +
      ...
      ggsave(file.path(dirs$plots_pub, paste0("panelA_forest_phaseAvg_with_SUS-RES_", run_scope, ".svg")),
...
plot_volcano_phaseavg <- function(df, n_labels = 10, pval_thresh = 0.05, effect_thresh = 1) {
  ...
  emm_df <- cbind(emm_df, pc) |>
    ...
    dplyr::mutate(
      pair = dplyr::case_when(
        ...
        (lhs == "sus" & rhs == "res") | (lhs == "res" & rhs == "sus") ~ "SUS-RES",
        TRUE ~ NA_character_
      )
    ) |>
  ...
  if (nrow(emm_df) > 0 && length(unique(emm_df$Sex)) > 0) {
    forest_plot <- ggplot(
      ...
      scale_color_manual(values = c("RES-CON" = "grey70", "SUS-CON" = "#e63a47")) +
  ...
  if (nrow(tc_df) > 0) {
    ...
    ribbons_plot <- ggplot(tc_df, aes(x = rel_time_h, y = mean_act, color = Group, fill = Group)) +
      ...
      geom_line(size = 0.8) +
      scale_color_manual(values = c("CON"="#c7c3bb", "RES"="#3e3c6f", "SUS"="#e63a47")) +
      scale_fill_manual(values = c("CON"="#c7c3bb", "RES"="#3e3c6f", "SUS"="#e63a47")) +
  ...
  if (nrow(diff_df) > 0) {
    ...
    heat_plot <- ggplot(diff_df, aes(x = rel_phase_h, y = factor(Change, levels = sort(unique(as.character(Change)))), fill = diff_SC)) +
      geom_tile() +
      scale_fill_gradient2(low = "#3e3c6f", mid = "white", high = "#d7191c", midpoint = 0) +
  ...
  # -------------------------------------------------
  # NEW: Phase-transition edge windows (last/first 2h)
  # -------------------------------------------------
  ...
  if (nrow(transitions_tbl) > 0) {
    phase_edge_long <- phase_edge_base %>%
      ...
      dplyr::inner_join(transitions_tbl, by = c("AnimalNum", "Change")) %>%
    ...
    ggsave(
      file.path(edge_plot_dir, paste0("phaseTransition_beforeAfter_", run_scope, ".svg")),
    ...
    openxlsx::write.xlsx(
      phase_edge_tp_animal,
      file.path(edge_tab_dir, paste0("phaseTransition_timepointAnimalMeans_", run_scope, ".xlsx")),
      rowNames = FALSE
    )
    openxlsx::write.xlsx(
      phase_edge_tp_summary,
      file.path(edge_tab_dir, paste0("phaseTransition_timepointSummary_", run_scope, ".xlsx")),
      rowNames = FALSE
    )
    ...
    ggsave(
      file.path(edge_plot_dir, paste0("phaseTransition_timepointLine_", run_scope, ".svg")),
    ...
    openxlsx::write.xlsx(
      ...
      file.path(edge_tab_dir, paste0("phaseTransition_deltaSummary_", run_scope, ".xlsx")),
      rowNames = FALSE
    )
    ...
    ggsave(
      file.path(edge_plot_dir, paste0("phaseTransition_delta_", run_scope, ".svg")),
      p_phase_edge_delta, width = 10.5, height = 6.5
    )

    # -------------------------------------------------
    # NEW: Stability plot — median across cage changes,
    ...
    openxlsx::write.xlsx(
      stability_animal,
      file.path(edge_tab_dir, paste0("phaseTransition_stability_medianChange_raw_", run_scope, ".xlsx")),
      rowNames = FALSE
    )

    group_colors <- c("CON" = "#3e3c6f", "RES" = "#c7c3bb", "SUS" = "#e63a47")
    ...
    for (sex_val in unique(stability_animal$Sex)) {
      ...
      ggsave(
      file.path(stability_plot_dir, paste0("stability_medianChange_raw_", sex_val, "_", run_scope, ".svg")),
    ...
    ggsave(
      file.path(stability_plot_dir, paste0("stability_medianChange_raw_allSexes_", run_scope, ".svg")),
      p_stability_all, width = 9, height = 6
    )

    # -------------------------------------------------
    # NEW: Stability single-timepoint profile (±2h bins)
    ...
    openxlsx::write.xlsx(
      stability_tp_raw,
      file.path(edge_tab_dir, paste0("phaseTransition_stability_timepointRaw_", run_scope, ".xlsx")),
      rowNames = FALSE
    )
    ...
    for (sex_val in unique(stability_tp_raw$Sex)) {
      ...
      stat_summary(aes(group = Group), fun = mean, geom = "line", linewidth = 1.0) +
      stat_summary(aes(group = Group), fun = mean, geom = "point", size = 2.1) +
      stat_summary(aes(group = Group), fun.data = mean_se, geom = "errorbar", width = 0.10, linewidth = 0.55) +
      scale_color_manual(values = group_colors) +
      scale_x_continuous(
        breaks = seq(-edge_window_bins, edge_window_bins - 1, by = 1),
        labels = function(x) sprintf("%+.1f", x / 2)
      ...
      ggsave(
      file.path(stability_plot_dir, paste0("stability_timepointRaw_", sex_val, "_", run_scope, ".svg")),
    ...
    ggsave(
      file.path(stability_plot_dir, paste0("stability_timepointRaw_allSexes_", run_scope, ".svg")),
...
p_animal_corr <- ggplot(animal_corr, aes(x = Group, y = pearson_r, fill = Group)) +
  ...
  scale_fill_manual(values = c("CON" = "#3e3c6f", "RES" = "#c7c3bb", "SUS" = "#e63a47")) +
  facet_wrap(~ Sex) +
  labs(
...
#   pal_grp <- c("CON"="#3e3c6f","RES"="#c7c3bb","SUS"="#e63a47")
#   
#   # START plot
