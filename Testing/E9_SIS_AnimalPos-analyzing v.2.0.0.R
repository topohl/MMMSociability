...
for (batch in batches) {
  ...
  for (system_id in unique_systems) {
    ...
    for (phase_number in phase_numbers) {
      ...
      # --- New: generate position heatmaps filtered by Group and Sex ---
      # Only run if relevant columns exist
      if ("Group" %in% colnames(data_system_phase)) {
      groups_in_phase <- unique(na.omit(data_system_phase$Group))
      for (g in groups_in_phase) {
        try({
        data_group <- data_system_phase %>% filter(Group == g)
        # build animal ids for this group and pad to 4
        animal_ids_group <- unique(data_group$AnimalID)
        while (length(animal_ids_group) < 4) animal_ids_group <- append(animal_ids_group, NA)
        ...
        count_position_list_g <- list(c(1, 0), c(2, 0), c(3, 0), c(4, 0),
                        c(5, 0), c(6, 0), c(7, 0), c(8, 0))

        # initialize and iterate through rows for the subgroup
        animal_list_g <- initialize_animal_positions(animal_ids_group, data_group, animal_list_g)
        initial_time_g <- animal_list_g[[1]][[2]]
        current_row_g <- 5
        total_rows_g <- nrow(data_group) + 1

        while (current_row_g != total_rows_g && current_row_g < total_rows_g) {
          previous_animal_list_g <- animal_list_g
          animal_list_g <- update_animal_list(animal_ids_group,
                            animal_list_g,
                            data_group,
                            initial_time_g,
                            current_row_g)
          elapsed_seconds_g <- animal_list_g[["data_temp"]][["elapsed_seconds"]]
          count_position_list_g <- update_position(previous_animal_list_g,
                               animal_list_g,
                               count_position_list_g,
                               elapsed_seconds_g)
          current_row_g <- animal_list_g[["data_temp"]][["current_row"]]
          initial_time_g <- animal_list_g[[1]][[2]]
        }

        # create heatmap using modified system id to indicate group
        heatmap_positions_group <- generateHeatMapPositions(count_position_list_g,
                                  batch,
                                  cageChange,
                                  paste0(system_id, "_Group_", g),
                                  phase,
                                  phase_number)
        systemHeatmaps_positions <- c(systemHeatmaps_positions, list(heatmap_positions_group))
        }, silent = TRUE)
      }
      }

      if ("Sex" %in% colnames(data_system_phase)) {
      sexes_in_phase <- unique(na.omit(data_system_phase$Sex))
      for (s in sexes_in_phase) {
        try({
        data_sex <- data_system_phase %>% filter(Sex == s)
        animal_ids_sex <- unique(data_sex$AnimalID)
        while (length(animal_ids_sex) < 4) animal_ids_sex <- append(animal_ids_sex, NA)
        ...
        count_position_list_s <- list(c(1, 0), c(2, 0), c(3, 0), c(4, 0),
                        c(5, 0), c(6, 0), c(7, 0), c(8, 0))

        animal_list_s <- initialize_animal_positions(animal_ids_sex, data_sex, animal_list_s)
        initial_time_s <- animal_list_s[[1]][[2]]
        current_row_s <- 5
        total_rows_s <- nrow(data_sex) + 1

        while (current_row_s != total_rows_s && current_row_s < total_rows_s) {
          previous_animal_list_s <- animal_list_s
          animal_list_s <- update_animal_list(animal_ids_sex,
                            animal_list_s,
                            data_sex,
                            initial_time_s,
                            current_row_s)
          elapsed_seconds_s <- animal_list_s[["data_temp"]][["elapsed_seconds"]]
          count_position_list_s <- update_position(previous_animal_list_s,
                               animal_list_s,
                               count_position_list_s,
                               elapsed_seconds_s)
          current_row_s <- animal_list_s[["data_temp"]][["current_row"]]
          initial_time_s <- animal_list_s[[1]][[2]]
        }

        heatmap_positions_sex <- generateHeatMapPositions(count_position_list_s,
                                  batch,
                                  cageChange,
                                  paste0(system_id, "_Sex_", s),
                                  phase,
                                  phase_number)
        systemHeatmaps_positions <- c(systemHeatmaps_positions, list(heatmap_positions_sex))
        }, silent = TRUE)
      }
      }
      # --- end new group/sex heatmaps generation ---
