...
# True dyadic RFID contact file generated by:
#   Analysis/05_build_dyadic_rfid_contacts.R
# If this file does not exist yet, the script falls back to animal-level summaries.
...
# some same-position contact in the bin. Set use_positive_dyad_weight = FALSE
# if you prefer quantile-thresholded edges for dense network visualizations.
...
if (!is.null(dyad_file) && file.exists(dyad_file)) {
  ...
  prox_col <- first_existing_col(dyad_raw, c("Weight", "Proximity", "proximity", "contact_fraction", "same_position_fraction", "Distance", "distance", "Interaction", "interaction_weight"), TRUE, "dyad proximity column")
  ...
  if (nrow(dyad) == 0) {
    ...
    dyad_summary <- edge_tbl %>%
      group_by(Group, Sex, Phase, CageChange, Batch, System, Focal, Partner) %>%
      summarise(
    ...
    if (requireNamespace("igraph", quietly = TRUE)) {
      ...
      graph_metrics <- edge_tbl %>%
        group_by(Group, Sex, Phase, CageChange, Batch, System, TimeIndex) %>%
        group_modify(~{
      ...
      node_summary <- node_metrics %>%
        group_by(Group, Sex, Phase, CageChange, Batch, System, AnimalNum) %>%
        summarise(
      ...
      p_density <- graph_metrics %>%
        ...
        labs(title = "Dynamic social-network density", x = "Time bin", y = "Density") +
        make_nature_theme()
      ...
      p_node <- node_summary %>%
        ...
        geom_jitter(width = 0.1, size = 0.8, alpha = 0.7) +
        facet_grid(Metric ~ Phase, scales = "free_y") +
        labs(title = "Animal-level network centrality", x = NULL, y = NULL) +
        make_nature_theme()
