...
#' specified environment. Shannon entropy is a measure of uncertainty or
#' randomness, which in this context, helps in understanding the spatial
#' behavior and movement patterns of animals.
#'
#' @details
#' This script is an integral component of the MMMSociability project,
#' aimed at studying sociability patterns among animals. The analysis
#' leverages data collected from various tracking devices and sensors,
#' processed through preliminary steps in the project pipeline. The
#' Shannon entropy metric provides insights into the complexity and
#' variability of animal movements, which can be crucial for ecological
...
#' 3. Entropy Calculation: Applying Shannon entropy formula to the
#'    positional data.
#' 4. Result Visualization: Generating plots and graphs to visualize
#'    entropy distribution.
#' 5. Output Generation: Saving the results and visualizations to the
#'    specified directory.
#'
#' @file
#' C:/Users/topohl/Documents/GitHub/MMMSociability/E9_SIS_AnimalPos-analyzing.r
#' @date January 2025
#' @authors
#' Tobias Pohl, Anja Magister
#'
#' @section Notes:
#' - Ensure that all required libraries (e.g., `entropy`, `ggplot2`) and
#'   data files are available and correctly referenced before executing
#'   the script.
#' - The script is intended to be executed within an R environment,
#'   preferably with R version 4.0 or higher.
#' - The results, including entropy values and visualizations, will be
#'   saved in the specified output directory for further analysis and
#'   reporting.
#'
#' @section References:
#' - For detailed methodology and data collection procedures, refer to
#'   the MMMSociability project documentation.
...
#'
#' @keywords
#' Shannon entropy, animal positions, spatial behavior, ecological
#' analysis, MMMSociability
...
for (i in seq_along(entropy_tibble_names)) {
  ...
  ifelse(name == "consec_cage_entropy",
         entropy_list <- consec_cage_entropy,
         entropy_list <- consec_animal_entropy)
...
columns_to_group <- list(c("Sex", "AnimalID", "Group"),
                         c("Sex", "AnimalID", "CageChange", "Group"),
                         c("Sex", "AnimalID", "Phase", "Group"),
                         c("Sex", "AnimalID", "Phase", "Group"))#phase group 2* bc of 2 phases
...
### CAGE ENTROPY ###
cage_ent_plot <- ggplot(data = cagePosEntropy, aes(x = Phase,
                                                   y = CageEntropy,
                                                   color = System)) +
  geom_point() +
