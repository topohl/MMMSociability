...
if (file.exists(data_file)) {
  ...
  data_file_rds <- "D:/MMMSociability/processed_data/data_lme_format/data_filtered_agg.rds"
  ...
  for (Change in changes) {
    for (Sex in sexes) {
      for (Phase in phases) {
        ...
        # Create time variable and scale ONLY the predictor (not response!)
        ...
        # Remove any rows with NAs in scaled time variable
        dsub <- dsub %>% dplyr::filter(!is.na(TH_scaled), !is.na(.data[[metric_name]]))
        
        if (nrow(dsub) == 0) {
          cat(paste(Sys.time(), Change, Sex, Phase, "SKIP: no valid data after scaling\n"), file = log_file, append = TRUE)
          next
        }
        ...
        # Fit model with ORIGINAL response units
        model <- quiet_singular(
          fit_lmer_adaptive(fixed_rhs, dsub, 
                            response_var = metric_name,  # ← Original units!
                            time_var = "TH_scaled", 
        ...
        if (inherits(model, "lmerMod")) {
          ...
          if (!is.null(conv_code) && conv_code != 0) {
            converged <- FALSE
            cat(paste(Sys.time(), Change, Sex, Phase, 
        ...
        # Skip emmeans if model didn't converge properly
        if (!converged) {
          cat(paste(Sys.time(), Change, Sex, Phase, "SKIP emmeans: convergence issue\n"), 
              file = log_file, append = TRUE)
          next
        }
        
        # Check for Group in fixed effects
        if (!any(grepl("^Group", names(fixef(model))))) next
        
        # EMM level contrasts (at TH_scaled = 0, which is the mean time)
        ...
        # Phase-average computations
        if (exists("model") && inherits(model, "lmerMod") && converged) {
          stat_k <- compute_phase_avg_for_model(model, Change, Sex, Phase, adjust_method = "holm")
          if (!is.null(stat_k$means) && nrow(stat_k$means)) {
            phase_avg_means_all[[length(phase_avg_means_all)+1]] <- stat_k$means
          }
          if (!is.null(stat_k$contrasts) && nrow(stat_k$contrasts)) {
            phase_avg_contr_all[[length(phase_avg_contr_all)+1]] <- stat_k$contrasts
          }
        }
      }
    }
