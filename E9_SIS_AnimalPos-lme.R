...
for(batch in batches){
  for(cageChange in cageChanges){
    ...
    active_data <- active_data[c('AnimalID', 'Sex', 'System', 'Group', 'Batch', 'Phase', 'SocialProx')]
    inactive_data <- inactive_data[c('AnimalID', 'Sex', 'System', 'Group', 'Batch', 'Phase', 'SocialProx')]
    total_closeness_table <- total_closeness_table[c('AnimalID', 'Sex', 'System', 'Group', 'Batch', 'Phase', 'SocialProx')]
...
for (PhaseValue in c("Active", "Inactive")) {
  ...
  mixed_model_results_file <- paste0("S:/Lab_Member/Anja/Git/MDC_Bachelor/E9_SIS_AnimalPos/lme/social_prox/mixed_model_results_", PhaseValue, ".txt")
  ...
  pairwise_results_file <- paste0("S:/Lab_Member/Anja/Git/MDC_Bachelor/E9_SIS_AnimalPos/lme/social_prox/pairwise_results_", PhaseValue, ".txt")
  ...
  emmeans_results_file <- paste0("S:/Lab_Member/Anja/Git/MDC_Bachelor/E9_SIS_AnimalPos/lme/social_prox/emmeans_results_", PhaseValue, ".txt")
...
total_active_hours_per_day_lineplot <-  ggplot(data=plotData_batch_prox_active, aes(x=fct_inorder(as_factor(ConsecActive)), y=Mean_prox, group=Group, color=Group))+
  geom_line()+
  scale_color_manual(values= c("sus"= "tomato", "res" = "lightgreen", "con" = "deepskyblue4"), name = "group")+
...
total_inactive_hours_per_day_lineplot <-  ggplot(data=plotData_batch_prox_inactive, aes(x=fct_inorder(as_factor(ConsecInactive)), y=Mean_prox, group=Group, color=Group))+
  geom_line()+
  scale_color_manual(values= c("sus"= "tomato", "res" = "lightgreen", "con" = "deepskyblue4"), name = "group")+
  scale_y_continuous("social proximity in h")+
...
for(cageChange in cageChanges){
  #read csv
  mouse_which_system_tibble <-  tibble(read_delim(paste0(working_directory,"/tables/", batch, "_", cageChange, "_mouse_which_system_tibble.csv"),delim = ",", show_col_types = FALSE))
  total_movement_table <- tibble(read_delim(paste0(working_directory,"/tables/", batch, "_", cageChange, "_total_movement_table.csv"),delim = ",", show_col_types = FALSE))
  
  #cut the last five columns
  total_movement_table <-select(total_movement_table,-starts_with("sys."))
  
  # melt tibble
  total_movement_table$Phase <-  1:nrow(total_movement_table)#rename both phases into numbers 
  total_movement_table<- melt(total_movement_table, id='Phase')
  ...
  #sort tibble into correct orde of columns
  total_movement_table <- total_movement_table[c('AnimalID', 'System', 'Group', 'Phase', 'Coilcross')]
  ...
  if(length(unique_phase_number)==0){
    #print("first cageChange")
    batch_cross <- bind_rows(batch_cross,total_movement_table)
  }else{
    last_consec <- max(unique_phase_number)
    #print(last_consec)
    total_movement_table <- total_movement_table%>%
      mutate(Phase = Phase+last_consec)
    #print(total_movement_table)
    #after that, add rows to general tibble
    batch_cross <- bind_rows(batch_cross,total_movement_table)
  }
...
###lme coil crossing
model6 <- lme(Coilcross~Group*Phase, data=batch_cross, random=~1|AnimalID/Group)
summary(model6)
